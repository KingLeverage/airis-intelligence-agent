...
**goat** — Cross-site recipe ranker (“best version of this dish”)

- Usage: `recipe-goat-pp-cli goat "<query>" [--limit N] [--sites …] --agent`
- Example: `recipe-goat-pp-cli goat "moist chocolate layer cake" --limit 8 --agent`
- Older docs may mention a `find` subcommand; current builds use **`goat`** for ranked multi-site results. Use **`search`** for title-only metadata without full fetches.
